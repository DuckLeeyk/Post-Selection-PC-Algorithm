...
class ResampledPC(PCAlgorithm):
    ...
    def aggregate_confidence_intervals(self, exposure, outcome):
        ...
        **实际中可用更多工具(如 networkx)辨别CPDAG->所有DAG，以获取最小调整集或父集等。**
        ...
        for idx in self.valid_indices:
            ...
            # (1) 找到一个符合后门准则的调整集合 S
            S_backdoor = self._find_backdoor_adjustment_set(G_m, exposure, outcome)

            # (2) 组装回归设计矩阵: X_exposure + X_S_backdoor
            regressors = [exposure] + list(S_backdoor)  # list(...) 确保可索引
            X_reg = self.X[:, regressors]
            y_reg = self.X[:, outcome]

            # (3) 找到 exposure 在 regressors 列表中的索引
            exposure_idx = regressors.index(exposure)

            # (4) 回归估计
            beta_hat, se_hat = self._estimate_linear_effect(X_reg, y_reg, exposure_idx)
    ...
    def _find_backdoor_adjustment_set(self, G_dag, i, j):
        """
        示例: 在 DAG G_dag 下，为 i->j 找一个满足后门准则的调整集 S。
        这里非常简化:
          1) 找到 i 的所有祖先 anc(i) (不含 i 自己),
          2) 同时找出 i 的所有父节点 pa(i),
          3) 排除 i 的后代 nodes,
          4) 返回 union(anc(i), pa(i)) - descendants(i).
        其实在后门准则下，还需确保所有从 i 出发且以箭头指向 i 的路径被阻断；该逻辑较复杂，
        此处仅示意，可根据实际需求改写。
        """
        anc_i = self._get_ancestors(G_dag, i)
        pa_i = set(np.where(G_dag[:, i] == 1)[0])  # 父节点
        desc_i = self._get_descendants(G_dag, i)

        # 这里做一个简单并集
        S = (anc_i | pa_i) - desc_i - {i}
        return S

    def _get_ancestors(self, G_dag, node):
        """
        返回节点node所有祖先集合(递归往前找).
        """
        res = set()
        frontier = [node]
        while frontier:
    ...
    def _get_descendants(self, G_dag, node):
        """
        返回节点node所有后代集合(递归往后找).
        """
        res = set()
        frontier = [node]
        while frontier:
...
class PCAlgorithmWithResampledTests(PCAlgorithm):
    ...
    def __init__(self, X, alpha=0.05, ordering=None, M=100, c_star=0.01, nu=0.01):
        ...
        # 新增一个缓存: 用于存储对每条(i,j)生成的一次性噪声
        # 例如: noise_cache[(i,j)] = float
        self.noise_cache = {}

    def _conditional_indep_test(self, i, j, S):
        """
        在计算出 Fisher's Z 后，为边(i,j)只加一次随机噪声:
          1) 若是第一次遇到 (i, j)，则生成 N(0,1) 并缓存
          2) 其余情况下直接复用
        然后比较 |Z(obs) + 噪声| <= shrink_tau(M)*z_thresh
        """
        # --------- 1) 首先计算 (i,j) 的 "observed Z" (不带随机干扰) ---------
        if len(S) == 0:
            corr = np.corrcoef(self.X[:, i], self.X[:, j])[0, 1]
        else:
            XS = self.X[:, list(S)]
        ...
        # 注意: 如在论文中, Fisher's Z 常写为 0.5 * ln((1+r)/(1-r))
        # 这里依旧保持原写法(缺少0.5因子), 若要完全匹配文献, 应加 0.5.
        z_obs = log((1 + corr) / (1 - corr)) if abs(corr) < 1 else np.inf
        df = self.n - len(S) - 3
        Z_obs = sqrt(df) * z_obs if df > 0 else z_obs

        # --------- 2) 为 (i, j) 从缓存中取/生成一次噪声 -----------
        # 统一键: (min(i,j), max(i,j)) 保证无论顺序如何都取同一份
        edge_key = (min(i, j), max(i, j))
        if edge_key not in self.noise_cache:
            self.noise_cache[edge_key] = np.random.normal(0, 1, 1)[0]

        Z_noise = self.noise_cache[edge_key]
        Z_sampled = Z_obs + Z_noise
